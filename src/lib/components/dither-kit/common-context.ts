 ...
 * Fields are getters into the root's runes state, so reads stay reactive.
 */
...
const COMMON_KEY = Symbol("dither-kit-common")

export function setCommonChart(common: CommonChart) {
  setContext(COMMON_KEY, common)
}

export function useCommonChart(): CommonChart {
  const ctx = getContext<CommonChart | undefined>(COMMON_KEY)
  if (!ctx) {
