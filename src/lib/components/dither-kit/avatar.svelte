...
<script lang="ts">
  import { cn } from "./lib"
  import { type PixelBloom, pixelBloomStyle } from "./pixel"
  ...
  let {
    ...
    mirror?: AvatarMirror
    /** Square size in px. Omit to size via class (e.g. `size-12`). */
    size?: number
  ...
  $effect(() => {
    ...
    if (!canvas) return
    void replayToken // dependency - bumping it replays the entrance
    return paintAvatar(
      canvas,
      bloomEl,
