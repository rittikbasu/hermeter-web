// requestAnimationFrame paint loop for the pie/donut canvas.
...
type LoopArgs = {
  ...
  cols: number
  rows: number
  width: number // plot css px
  height: number
  state: Ref<PolarChartState>
}

/**
 * Each backing pixel is mapped back to plot space, tested for its slice by
 * angle, and filled with the ordered-dither scatter - dense at the outer edge,
 * thinning toward the centre - capped by a bright arc on the rim. The pie
 * sweeps in clockwise on mount; the hovered slice bulges outward with a
 * brighter rim while the rest dim. Returns a cleanup that cancels the loop.
 */
export function startPieLoop({
  canvas,
  bloomCanvas,
  cols,
  rows,
